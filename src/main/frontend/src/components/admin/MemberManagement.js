...
const AddEditMemberModal = ({ show, onClose, isEditing, newMemberData, handleNewMemberDataChange, handleSaveMember }) => {
    if (!show) return null;
    return (
        <div className={memberStyles.modalOverlay}>
            <div className={memberStyles.modalContent}>
                ...
                <div className={memberStyles.modalActions}>
                    ...
                    <button onClick={onClose} className={memberStyles.modalSecondaryBtn}>취소</button>
                </div>
            </div>
        </div>
    );
...
const IndividualCouponModal = ({ show, onClose, member, availableCoupons, selectedCouponForIndividual, setSelectedCouponForIndividual, handleDistributeCouponToIndividual }) => {
    if (!show) return null;
    return (
        <div className={memberStyles.modalOverlay}>
            <div className={memberStyles.modalContent}>
...
const CouponDistributionModal = ({ show, onClose, selectedMembersCount, availableCoupons, selectedCouponToDistribute, setSelectedCouponToDistribute, handleDistributeCoupon }) => {
    if (!show) return null;
    return (
        <div className={memberStyles.modalOverlay}>
            <div className={memberStyles.modalContent}>
...
function MemberManagement() {
    ...
    const handleSelectAll = (e, currentItems) => { // currentItems 인자 추가
        if (e.target.checked) {
            setSelectedMembers(currentItems.map(member => member.id)); // 현재 페이지의 회원만 선택
        } else {
            setSelectedMembers([]);
        }
    };
                        ...
                        <tr>
                            <th>
                                <input
                                    ...
                                    checked={currentItems.length > 0 && currentItems.every(member => selectedMembers.includes(member.id))} // currentItems 기준으로 체크
                                />
                            </th>
                            <th>회원ID</th>
                            <th>닉네임</th>
    ...
    return (
        <div className={styles.app}>
            ...
            <CouponDistributionModal
                ...
                selectedCouponToDistribute={selectedCouponToDistribute}
                setSelectedCouponToDistribute={setSelectedCouponToDistribute}
            ...
            <IndividualCouponModal
                ...
                availableCoupons={availableCoupons}
                selectedCouponForIndividual={selectedCouponForIndividual}
                setSelectedCouponForIndividual={setSelectedCouponForIndividual}
                handleDistributeCouponToIndividual={handleDistributeCouponToIndividual}
            />
