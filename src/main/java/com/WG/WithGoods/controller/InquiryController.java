...
@RestController
@RequestMapping("") // Remove class-level /api prefix
...
public class InquiryController {
    ...
    /** 일반문의(JSON) 등록 */
    @PostMapping(path="/inquiries", consumes=MediaType.APPLICATION_JSON_VALUE)
    ...
    public ResponseEntity<String> createEstimateInquiry(
            ...
            @RequestParam("secret") String secretStr,
            @RequestParam(value="designFile", required=false) MultipartFile designFile
    ) {

        String writer = (principal != null) ? principal.getName() : "GUEST";
        Boolean secret = Boolean.parseBoolean(secretStr);

        // type을 enum으로 직접 지정
        InquiryType type = InquiryType.ESTIMATE;

        inquiryService.createEstimateInquiry(
                writer, title, customerName, contact,
                product, quantity, message,
                password, secret, designFile, type
        );
    ...
    /** 내 견적 문의 조회 */
    @GetMapping("/estimate/my")
    public List<InquiryResponseDto> myEstimates(HttpSession session) {
        String user = (String) session.getAttribute("username");
        return inquiryService.findEstimatesByUser(user);
    }
    ...
    // 승인 처리
    @PutMapping("/inquiries/{id}/approved")
    ...
    // 거절 처리
    @PutMapping("/inquiries/{id}/rejected")
